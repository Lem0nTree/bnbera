...
export function buildBoundedPancakeSwap(input: { readonly policy: ScopedPolicy; readonly wallet: Address; readonly quotedOut: bigint; readonly nowUnix: number; readonly deadlineUnix: number; readonly allowReleaseDisabled?: boolean }): { readonly request: ActionRequest; readonly calldata: `0x${string}`; readonly minOut: bigint } {
  const profile = pancakeSwapProfile(input.policy.chainId);
  if (profile.releaseEnabled === false && input.policy.chainId === 56 && input.allowReleaseDisabled !== true) throw new AltanaBoundaryError("CALL_NOT_ALLOWED", "Mainnet swap execution is release-disabled.");
  if (input.quotedOut <= 0n || input.deadlineUnix <= input.nowUnix || input.deadlineUnix - input.nowUnix > 120) throw new AltanaBoundaryError("CALL_NOT_ALLOWED", "Swap quote or deadline is invalid.");
