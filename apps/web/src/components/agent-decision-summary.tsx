...
export function AgentDecisionSummary({ agent }: { readonly agent: MarketplaceAgentReadModel }) {
  ...
  const observed = uptime.status === "observed" && uptime.attemptedChecks > 0;
  ...
  return <section className="decision-grid" aria-label="Agent track record">
    <article className="decision-card decision-card--health"><span className="eyebrow">Service heartbeat</span><h2 className="heartbeat" data-online={heartbeatLabel(agent) === "Online"}><i />{heartbeatLabel(agent)}</h2><p>Last checked {formatObservedAt(agent.health.observedAt)}</p>
