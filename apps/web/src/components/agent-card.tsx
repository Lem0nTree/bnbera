...
export function AgentCard({ agent }: { readonly agent: MarketplaceAgentReadModel }) {
  ...
  return <article className="agent-row agent-row--marketplace">
    <Link prefetch={false} className="agent-row__art" href={`/agents/${agent.slug}`} aria-label={`View ${agent.name}`}><AgentAvatar category={agent.category} name={agent.name} /></Link>
    <div className="agent-row__content">
      <h3><Link prefetch={false} href={`/agents/${agent.slug}`}>{agent.name}</Link></h3>
    ...
    <div className="agent-row__actions"><Link prefetch={false} className="button button--primary" href={`/agents/${agent.slug}${agent.activation.enabled ? "#hire" : ""}`}>{agent.activation.enabled ? "Hire agent" : "View agent"} <span aria-hidden="true">↗</span></Link></div>
  </article>;
}
