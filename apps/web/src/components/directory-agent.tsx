...
export function DirectoryAgentRow({agent}:{readonly agent:MarketplaceAgentReadModel}) {
  ...
  return <article className="agent-row directory-row">
    <Link className="agent-row__art" href={`/agents/${agent.slug}`} aria-label={`View ${agent.name}`}><AgentAvatar category={agent.category} imageUrl={data.imageUrl} name={agent.name}/></Link>
    <div className="agent-row__content"><div className="directory-row__title"><h3><Link href={`/agents/${agent.slug}`}>{agent.name}</Link></h3><span className={`chain-chip${agent.identity.chainId===97?" chain-chip--testnet":""}`}><span aria-hidden="true">◆</span> {agent.identity.chainId===56?"BNB Chain":"Testnet"}</span></div><p className="agent-row__description">{agent.description}</p><div className="directory-row__signals"><span title="Feedback indexed by 8004scan; permissionless, not verified BNBEra purchases">☆ {data.feedback.count?.toLocaleString()??"—"} feedback{data.feedback.average!==null?` · ${data.feedback.average.toFixed(1)}/100`:""}</span><span title={`8004scan profile update: ${formatObservedAt(data.vendorUpdatedAt)}`}>◷ Updated {relativeObservation(data.vendorUpdatedAt)}</span><span className={checked?"signal-positive":""}><i/>{checked ? `${checked} protocol verified` : "No fresh verified protocol"}</span></div><div className="directory-row__protocols">{data.services.slice(0,4).map(service=><DirectoryServiceStatus key={`${service.name}:${service.url}`} service={service} data={data}/>)}{agent.category!=="uncategorized"&&<span>{categoryLabel(agent.category)}</span>}</div></div>
    <div className="directory-row__score"><DirectoryScore score={data.scores.overall}/><span>{data.sourceLabel ? "Vendor score unavailable" : "8004scan score"}</span>{data.scores.activity !== null && <small title={`8004scan activity score, out of 100. Scored ${formatObservedAt(data.scores.observedAt)}; not a completed-job count.`}>Activity {data.scores.activity.toFixed(1)}/100</small>}</div>
    <div className="agent-row__actions"><Link className="button button--primary" href={`/agents/${agent.slug}${agent.activation.enabled ? "#hire" : ""}`}>{agent.activation.enabled ? "Review mainnet offer" : "Explore agent"} <span aria-hidden="true">↗</span></Link><small>{agent.activation.enabled ? "Fresh signed quote required" : "Hiring unavailable"}</small></div>
    <div className="directory-row__evidence"><span>{data.services.length} advertised {data.services.length === 1 ? "service" : "services"}</span><span title={formatObservedAt(latestCheck)}>{latestCheck ? `Service check ${relativeObservation(latestCheck)}` : "Service checks pending"}</span><span>{agent.pricing.amountAtomic ? agent.pricing.label : "Price not verified"}</span><span>ERC-8004 #{agent.identity.agentId}</span></div>
  </article>;
}
  ...
  return <div className="page-shell directory-profile">
    ...
    <div className="directory-profile__layout"><div className="directory-profile__main"><section className="directory-overview"><span className="eyebrow">Meet your next agent</span><p>{agent.description}</p><div className="directory-service-tags">{data.protocols.map(protocol=><span key={protocol}>{protocol}</span>)}{agent.category!=="uncategorized"&&<span>{categoryLabel(agent.category)}</span>}</div></section>
      ...
      {agent.activation.method === "erc8183" && <section className="directory-section directory-hire" id="hire" aria-labelledby="directory-hire-title">
        <div className="directory-hire__heading"><span className="eyebrow">Put this agent to work</span><span className="directory-hire__network">BNB Chain · Mainnet</span></div>
        <h2 id="directory-hire-title">Hire with a signed mainnet offer</h2>
        <p className="directory-hire__intro">Describe your task, review your signed quote, and fund the work from your wallet.</p>
        <div className="directory-hire__price"><span>Observed price</span><strong>{agent.pricing.label}</strong><small>Your fresh signed quote sets the final amount.</small></div>
        <ol className="directory-hire__steps" aria-label="Hiring process"><li><span>01</span> Review quote</li><li><span>02</span> Fund escrow</li><li><span>03</span> Review result</li></ol>
        <ActivationPanel activation={agent.activation} detail targetChainId={56} identityKey={agent.id} runBundle={agent.evidence.runBundle} />
      </section>}
    ...
    </div><aside className="directory-profile__aside"><section className="directory-availability"><DirectoryAvailability agent={agent}/><div className="directory-aside-divider"/><div className="directory-aside-kv"><span>Completed BNBEra hires</span><strong>{agent.metrics.completedJobs.completedCount??"—"}</strong></div><div className="directory-aside-kv"><span>Verified buyer reviews</span><strong>{agent.metrics.reputation.verifiedPurchases.count??"—"}</strong></div>{agent.activation.enabled ? <a className="button button--primary" href="#hire">Review mainnet offer <span aria-hidden="true">↗</span></a> : <><button className="button button--primary" disabled>Hire unavailable</button><details className="directory-unavailable-reason"><summary>Why unavailable?</summary><p>{agent.activation.reason}</p></details></>}</section><a className="directory-source-link" href={data.sourceUrl} target="_blank" rel="noreferrer">View source profile ↗</a></aside></div>
  </div>;
}
