...
export function DirectoryAgentRow({agent}:{readonly agent:MarketplaceAgentReadModel}) {
  ...
  return <article className="agent-row directory-row">
    ...
    <div className="agent-row__actions"><div className="directory-row__price"><span>Last quoted</span><strong>{agent.pricing.amountAtomic ? priceDisplayLabel(agent.pricing.label).replace(/^Last quoted\s+/i, "") : "Not verified"}</strong></div><Link className="button button--primary" href={`/agents/${agent.slug}${agent.activation.enabled ? "#hire" : ""}`}>{agent.activation.enabled ? "Hire" : "Explore agent"} <span aria-hidden="true">↗</span></Link><small>{agent.activation.enabled ? "Review a fresh signed quote" : "Hiring unavailable"}</small></div>
