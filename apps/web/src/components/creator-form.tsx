...
export function CreatorForm() {
  ...
  async function submit(formData: FormData) {
    ...
    const response = await fetch("/api/creator/drafts", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
      idempotencyKey: crypto.randomUUID(), name: formData.get("name"), slug: formData.get("slug"), description: formData.get("description"), protocol: "pancakeswap-v2", publicationConsent: formData.get("publicationConsent") === "on"
    }) });
  ...
  return <form action={submit} className="activation-panel">
    ...
    <button type="submit">Save one-shot swap draft</button>
