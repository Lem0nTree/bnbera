    ...
    <div className={`commerce-journey${walletOnly ? " commerce-journey--wallet-only" : ""}${compact ? " commerce-journey--compact" : ""}${compact && operationId === null && quote === null ? " commerce-journey--compose" : ""}`} data-testid="commerce-journey">
      ...
      {operationId === null && quote !== null && <div className="commerce-journey__quote">
        ...
        {quote.externalSeller && <>
          ...
          <label><input type="checkbox" checked={riskConfirmed} onChange={event => setRiskConfirmed(event.target.checked)} /> I accept this seller's delivery risk, the selected APEX policy (seven days from submission before payout; buyer approval is not a veto), and token issuer controls.</label>
        </>}
      ...
      {buyerChainId === 56 && job && !completed && ["funded", "submitted"].includes(job.job.state) && <div className="commerce-journey__delivery">
        <p className="eyebrow">Provider delivery</p><p>Once escrow is funded, request delivery from the agent. You can read its verified report as soon as it is retrieved; the payout window does not delay access to the result.</p>
      ...
      {submitted && <div className="commerce-journey__decision">
        ...
        {buyerChainId === 56 && <p>The selected APEX policy prevents early payout. Anyone can settle after seven days from submission unless a buyer dispute reaches the required rejection votes. A dispute alone does not stop payout; enough authorized votes can reject the job earlier. BNBEra does not send settlement transactions automatically.</p>}
        ...
        <div className="detail-actions"><button className="button button--primary" type="button" disabled={busy || !resultReviewed || authority === null || !walletAuthenticated || chainId !== buyerChainId || (job?.settlementGate!==undefined&&job.settlementGate.status!=="ready")} onClick={() => void decide("approve")}>Approve and request settlement</button><button className="button button--ghost button--small" type="button" disabled={busy || authority === null || !walletAuthenticated || chainId !== buyerChainId} onClick={() => void decide("dispute")}>Dispute result</button></div>
      </div>}
