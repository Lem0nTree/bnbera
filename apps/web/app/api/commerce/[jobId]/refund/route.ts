...
export async function POST(
  ...
  try {
    ...
    const input = await parseCommerceJson(request, commerceRefundRequestSchema);
    const composition = await getCommerceComposition(request);
    const result = await composition.refund(request, jobId, input.idempotencyKey);
