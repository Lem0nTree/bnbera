...
### Implemented read seam: account-wide hired jobs

P0 read seam is implemented: `/hired` can use authenticated `GET /api/commerce/jobs?cursor=…&limit=…`. The endpoint derives buyer ownership exclusively from the validated server session; it accepts no address lookup, uses existing PostgreSQL records, caps pages at 50, and orders deterministically by update time and job ID. It returns job/reservation and protocol IDs, exact historical agent identity/version, safe name/slug when available, application and canonical lifecycle states, quoted amount/token/network, created/updated/expiry times, result/settlement/review availability, latest public operation ID/status/step/hash, a safe next action, and an opaque next cursor. It excludes raw tasks, manifests, operation contexts and other private payloads. Existing detail reads enforce authorization independently. The `/hired` UI route remains to be implemented.
