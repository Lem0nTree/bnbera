...
| T7 | Disabled scaffolding/integration seam | One fixed Studio template has restart-safe progression and tuple-bound G1/G2 reconciliation seams. ERC-8004 registration remains a browser-signature/finalized-read requirement; no server operator key is used. | Perform useful create -> browser-signed register -> finalized ingest/publish -> funded hire/activation, and prove duplicate/revoke behavior. |
...
| G3 — No-code Creator | Planned; disabled scaffolding present | T6–T7 | The bounded integration seams remain fail-closed without a browser caller, reviewed T6 gateway/sink, useful execution and browser-signed ERC-8004 action. No G3 claim is made. |
