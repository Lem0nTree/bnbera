...
| T6 | Disabled scaffolding/integration seam | The Creator composition has authenticated status/revoke UI/API seams and a PostgreSQL secret-reference-only adapter, but no browser caller or live reviewed gateway/sink is configured. | Verify the browser-controlled grant/handoff with the Studio-owned sink, then prove allowed, revoked, expired and over-cap behavior on testnet. |
| T7 | Disabled scaffolding/integration seam | Studio template 1.1.0 has restart-safe progression, tuple-bound G1/G2 reconciliation seams, and bounded Creator-owned configuration: curated testnet native pairs, input, slippage, quote freshness and deadline. ERC-8004 registration remains a browser-signature/finalized-read requirement; no server operator key is used. | Perform useful create -> browser-signed register -> finalized ingest/publish -> funded hire/activation, and prove duplicate/revoke behavior. The runtime remains plan-only until the Studio session-permission mismatch is resolved. |
...
| G3 — No-code Creator | Planned; disabled scaffolding present | T6–T7 | The bounded integration seams remain fail-closed without a browser caller, reviewed T6 gateway/sink, useful execution and browser-signed ERC-8004 action. Template 1.1.0 exposes only curated configuration, not arbitrary contracts/routes/calldata. No G3 claim is made. |
