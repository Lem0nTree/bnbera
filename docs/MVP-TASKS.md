...
## Final 24-hour sprint plan (20h implementation/acceptance + 4h submission)

The remaining window is timeboxed from sprint kickoff. Reserve the final four hours exclusively for submission assembly, so no new feature or migration may enter that window. The first four hours start with the existing UI and its real data path; use the current Next.js components and routes, not a redesign framework, new UI kit or parallel application. T8/G4 testnet/local evidence and the active T6/T7 Creator evidence are accepted for their documented scopes and should be surfaced/rehearsed, not reopened. T5/G2 interactive WalletConnect browser acceptance and T3's stable public preview are the critical open claims.

| Countdown | Task/gate and owner | Must-have exit evidence | Cut or defer if late |
| --- | --- | --- | --- |
| 0:00–0:30 | T9 kickoff / QA: freeze the starting SHA, inspect the existing env and retained-data assumptions, and record the exact G2/T3 blockers. | Build and focused web/commerce checks have a known starting result; `releaseEnabled=false`, testnet-only flags and secret-reference boundaries are unchanged. | Any cosmetic cleanup, dependency refresh or standards-lock change. |
| 0:30–4:00 | T2/T5 WEB, G1/G2 UI integration first: wire the existing browse, search/filter, four category, compare and detail surfaces to current API data and the hire CTA. | A reviewer can find an agent, understand category/capability/source/time/availability, distinguish advertised from tested service, see honest unknown reputation/jobs/price, open the task/quote state, and see useful loading/error/reload states. No fabricated metrics or empty-category claims. | Visual redesign, new components that duplicate existing ones, semantic-provider release, additional supply integrations and non-blocking analytics. |
| 4:00–10:00 | T5 / G2 critical browser run: use only wagmi's `walletConnect` connector and chain-97 SIWE/EOA boundary. Reverify current identity 2206/version/category/endpoint, then run the sequential buyer path. | Through the browser: connect and bind EOA/SIWE; `createJob` receipt yields the actual ID; register, set budget, exact bounded approval and fund; provider returns a useful result; exact digest/manifest and operation events reconcile; buyer explicitly settles (or disputes/refunds only under protocol rules); completed-job count and one verified-purchase review appear. Reload, duplicate-submit, account/chain change and one unknown-step recovery are demonstrated. | A second wallet connector, Altana buyer wallet/passkey, x402/B402, a second escrow path or any workaround that bypasses operation/event reconciliation. |
| 10:00–12:00 | T6/T7/G3 and T8/G4 evidence UI: add the smallest panels/links needed to expose already accepted Creator and Greenfield evidence in the existing detail/dashboard flow. | Creator shows bounded authority status/expiry/revoke and the audited Studio/identity evidence without secrets. Greenfield shows both public links, hashes/status/readback time, exact agent/version/job binding and the historical label; zero seal transaction is shown as unavailable. Browsing/hiring remain usable if storage evidence is unavailable. | New Creator templates, a new deployment, Greenfield mirroring, production/mainnet writes or private artifact publication. |
| 12:00–14:00 | T3/T5/T9 blocking responsive/accessibility pass. | Desktop and narrow viewport have no blocking overflow or clipped CTA; keyboard users can reach/connect/quote/fund/result/approve/review controls with visible focus; form labels, status text, errors and unknown/pending states are accessible and not color-only. | Pixel-level polish, animation, typography refresh, broad device matrix and non-blocking WCAG cleanup. |
| 14:00–17:00 | T3/T9 stable deployment near the end: build an immutable accepted SHA through the existing authorized HTTPS topology, retain PostgreSQL and install/verify the two cron jobs. | Public origin serves the deployed SHA; browser -> API -> PostgreSQL works after restart; browse health/freshness and deterministic fallback are honest; cron status is recorded; PostgreSQL is not exposed. | New hosting architecture, DNS experiments, database moves, migration rewrites or production/mainnet enablement. If the public origin cannot be stabilized, retain the local/testnet evidence and label public preview as unaccepted. |
| 17:00–20:00 | T5/T9 final acceptance and evidence capture. | Re-run the shortest complete public journey at the deployed SHA: find/compare/understand, accepted G2 browser result if achieved, Creator grant/revoke evidence, and both Greenfield links. Capture current gate pass/fail, exact IDs/transactions/digests/URLs, category gaps and at least three paired Agent Advantage runs (including trading, stock/equities or security). | Further implementation after the 20:00 freeze; only correction of a submission typo is allowed in the reserved window. |
| 20:00–24:00 | T9 submission freeze (submission only). | Assemble the demo script/video or walkthrough, deployed URL/SHA, evidence references, required Agent Advantage Report, honest unresolved blockers and rollback/disable commands; verify the submitted claims match accepted gate scope. | No feature work, migration, dependency update, new canary, paid resource or release-flag change. |

### Hard decision checkpoints

Use these checkpoints to protect the submission reserve and keep claims honest:

- **At 4:00 (UI exit):** the existing marketplace journey must be navigable from browse/detail to the hire CTA. If it is not, cut polish and keep the remainder on the core route, task form, recovery and error states.
- **At 7:00 (funding checkpoint):** WalletConnect/SIWE, actual `JobCreated` ID, router registration, budget, exact approval and funding must be either confirmed in the browser or clearly blocked by an external wallet/RPC condition. If blocked, stop expanding scope, preserve all no-rebroadcast safeguards, and prepare the accepted-scope fallback; never call G2 passed from the operator EOA or tunnel canary.
- **At 10:00 (G2 decision):** only a browser-bound useful result, exact evidence, explicit terminal buyer decision, review and recovery evidence can change G2 from pending. Otherwise keep `releaseEnabled=false`, record G2 as pending in the submission, and demonstrate only the accepted supporting evidence.
- **At 14:00 (scope lock):** Creator/Greenfield evidence must be visible and responsive/accessibility blockers fixed. Do not reopen completed T6/T7/T8 implementation to chase polish.
- **At 17:00 (deployment decision):** if the immutable public origin and persistence/cron smoke are not green, do not introduce new infrastructure; submit the local/testnet evidence with the public-preview limitation explicitly stated.
- **At 20:00 (freeze):** stop implementation. T9 owns only evidence reconciliation and submission assembly for the final four hours.

### Must-have claim set and late-cut policy

The must-have submission claim set is: an existing marketplace browse/detail/compare journey with truthful metrics; the interactive WalletConnect G2 journey if and only if its browser evidence passes; the already accepted active T6/T7 Creator grant/revoke/deployment scope; the already accepted T8 Greenfield testnet/local links; and a stable public deployment only if T3 smoke passes at the deployed SHA. T1/T2 retained/local acceptance remains valid but is not silently promoted to public or four-category acceptance.

If time expires, cut in this order: cosmetic polish and non-blocking accessibility, extra category depth or new agents, semantic release, x402/B402, new Creator templates, Greenfield production/mirroring, and any broad refactor. Do not cut or weaken wallet/event reconciliation, exact result verification, buyer approval/dispute rules, identity/version joins, secret handling, persistence/reload recovery or truthful status labels. A failed checkpoint narrows the claim; it does not authorize a fabricated pass. This plan schedules work only and does not itself enable a release or launch a new paid/mainnet operation.
