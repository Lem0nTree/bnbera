...
Active backlog for [MVP-MASTER-PLAN.md](MVP-MASTER-PLAN.md). Merged GitHub `main` baseline: `869c5873d8fdedb3263520c295f59e85f05db00a`; PRs #20–#40 are merged, including T8 PRs #31–#34, buyer-owned jobs API PR #38, cohesive P0 UI PR #39 and P1 UI PR #40. PR #40 validation is green; the active T8 Greenfield testnet/local API-detail scope and T6/T7 Creator scope remain accepted only with their documented qualifiers. Reconciled 2026-09-09. Two implementation agents maximum; coordinator reviews each handoff before dispatching its dependent replacement.

Status key: **Completed** means the source and stated retained/live evidence meet the accepted scope; **Implemented pending acceptance** means the source path is merged and supporting evidence exists but an explicit browser or other gate requirement remains open; **Blocked** names an external or safety prerequisite; **Planned** has no accepted implementation yet. A branch test is never a substitute for a live gate. Scope qualifiers distinguish retained/local acceptance from a public or paid claim.
...
## Final 24-hour sprint plan (remaining 20h + 4h submission reserve)

At this baseline the cohesive P0/P1 UI, authenticated buyer-history API/UI, T8 local/API-detail evidence and active T6/T7 evidence are merged. Reserve the final four hours for submission only; no new feature, migration, paid resource, mainnet write or release-flag change enters that window.

| Window | Owner and focus | Exit evidence | Cut/hold if late |
| --- | --- | --- | --- |
| 0:00–0:30 | T9 kickoff: freeze `869c587`, inspect environment/retained DB and record G2/T3 blockers. | Known build/focused-check baseline; `releaseEnabled=false`, testnet-only flags and secret-reference boundaries unchanged. | Cosmetic work and dependency changes. |
| 0:30–7:00 | T5/G2: real browser run through the single wagmi `walletConnect` connector and chain-97 SIWE/EOA boundary. | Actual `JobCreated` ID; register/budget/exact approval/fund; useful 2206 result; exact digest/manifest and events; explicit settle/dispute/refund rules; verified review; reload, duplicate, account/chain-change and unknown-step recovery. | Any second connector/rail, Altana buyer wallet/passkey, or workaround around event reconciliation. |
| 7:00–10:00 | T3/T9: stable authorized deployment plus cron and API→PostgreSQL smoke. | Immutable public SHA, restart persistence, two cron jobs, honest freshness/fallback and private PostgreSQL; otherwise retain local/testnet labels. | New hosting architecture, DNS experiments, migrations or mainnet writes. |
| 10:00–16:00 | T5/T9: current evidence capture and Agent Advantage report. | Current gate table, exact IDs/transactions/digests/URLs, category gaps and three paired runs including trading, stock/equities or security. | New supply, semantic release, Creator templates, Greenfield mirroring or cosmetic polish. |
| 16:00–20:00 | T9: integrated QA and claim freeze. | Desktop/mobile/keyboard/reduced-motion checks, browse→detail→hire and `/hired` resume path, Creator revoke and both Greenfield links; unresolved blockers are explicit. | No implementation after freeze. |
| 20:00–24:00 | T9 submission only. | Demo script/video or walkthrough, deployed URL/SHA if accepted, evidence references, Agent Advantage Report, honest limitations and rollback/disable commands. | No feature, migration, canary, paid resource or release enablement. |

The claim set remains limited to accepted retained/local, testnet/local API-detail and documented Creator scopes unless the corresponding browser/deployment evidence passes. A failed checkpoint narrows the claim; it never authorizes a fabricated pass.

## Current delivery status

This table distinguishes code merged to GitHub `main`, retained/testnet evidence, and the acceptance still required for a public claim. A task is not complete merely because a supporting package or unit test exists.

| Task | Status | Purpose and what is complete | Missing / genuine blocker |
| --- | --- | --- | --- |
| T1 | Completed (retained scope) | Durable, locked discovery and health cron, cursor/retry state, bounded work, restart safety and two-minute freshness expiry are accepted on retained data. | **Pending:** reinstall both jobs from the immutable preview checkout and capture final-host evidence; this does not reopen retained acceptance. |
| T2 | Completed (retained scope; follow-ups pending) | ERC-8004 enrichment, evidence-backed categories, vectors, deterministic fallback, marketplace projections, reputation provenance and the cohesive browse/search/filter/detail/compare UI from PRs #39–#40 are merged and tested. Missing feedback remains unknown, not fabricated. | **Pending:** grow qualified four-category supply. **Blocked:** semantic release until the candidate standards lock has production evidence. |
| T3 | Completed (retained/local scope) | Production build/read model, API/SSR browse, filters, categories, compare, detail, restart persistence, stale-health and fallback checks pass for the retained/local scope. | **Pending:** stable authorized HTTPS, deployed-browser acceptance, cron reinstall and final API→PostgreSQL smoke; no public deployment is claimed. |
| T4 | Completed (backend/canary scope; G2 pending) | Pinned ERC-8183 boundary, PostgreSQL lifecycle, operation reconciliation and authorized chain-97 hire→submit→explicit approval→settlement canary pass. | **Blocked until T5/G2:** browser-bound useful work by a discovered agent is still unproven; keep `releaseEnabled=false`. |
| T5 | Implemented pending acceptance | Buyer quote/funding/result/review composition, WalletConnect-only EOA/SIWE path, operation recovery and exact result projection are merged. PR #38 adds authenticated buyer-scoped cursor history; PR #39 adds cohesive P0 marketplace/Creator/hired UI and toasts; PR #40 adds comparison guidance, identity continuity, readable result summaries and narrow/keyboard/reduced-motion fixes. Web checks are 182/182, focused result checks 5/5; typecheck, lint, build and UI browser checks pass. | **Pending browser acceptance:** real WalletConnect pairing, chain-97 SIWE, sequential createJob/actual-ID→register→budget→exact approve→fund, useful 2206 result, exact evidence, explicit terminal decision, review and reload/duplicate/account-change/unknown recovery. Operator EOA/tunnel evidence is not a substitute; `releaseEnabled=false`. |
| T6 | Completed (active documented browser-canary scope) | User-controlled Altana grant/revoke, persisted `revoked` state, explicit over-cap/expiry denial and safe secret-reference boundary are evidenced. | No remaining T6 acceptance in the documented scope; keep the Studio trial temporary and never expose session material. |
| T7 | Completed (active documented Studio-trial scope) | BNB Agent Studio tBNB→BUSD creation, exact digest/identity registration, card, publication, category/vector follow-through and pre-Studio denial are evidenced. | No remaining T7 acceptance in the documented scope; preserve the audited allowlist and keep G2 separate. |
| T8 | Completed (testnet/local API-detail scope) | Greenfield bucket `25041`, historical version-11 profile and settled job-1103 bundle are public-read, hash-matched, persisted and retry-safe; local API/detail shows both exact-version links and honest unavailable seal-hash semantics. | **T9 only:** stable public deployment/deployed-browser walkthrough and submission evidence; no production/mainnet claim. |
| T9 | Planned | Owns final public walkthrough, current evidence capture and submission; P0/P1 UI implementation and focused checks are already merged under T2/T5/T6/T7. | **Pending:** stable deployment/cron/API-DB smoke, interactive G2 if achieved, current gate evidence, three Agent Advantage pairs and submission package. |

### Gate mapping

| Gate | Status | Tasks | Current acceptance boundary |
| --- | --- | --- | --- |
| G1 — Persistent marketplace | Completed (retained/local scope only) | T1–T3 | Durable discovery→enrichment→category→vector→publication→browse/detail/compare is accepted on retained/local data. **Missing:** stable authorized HTTPS/deployed-browser alignment and final-host cron/API→DB smoke; no public G1 claim. |
| G2 — Paid hiring | Implemented pending acceptance | T4–T5 | Backend, WalletConnect-only buyer path, account-wide job API/UI, operator EOA useful result/settlement/review and recovery guards are merged. **Missing/blocker:** real interactive WalletConnect pairing, SIWE, browser-bound useful result, exact sequential funding, terminal decision/review and recovery evidence; `releaseEnabled=false`. |
| G3 — No-code Creator | Completed (active documented scope; temporary Studio trial) | T6–T7 | Browser grant/revoke, bounded Studio creation, exact identity reconciliation, publication/category/vector follow-through and pre-Studio denial are evidenced. This does not claim G2 WalletConnect acceptance. |
| G4 — Greenfield | Completed (testnet/local API-detail scope) | T8 (T9 owns final walkthrough) | Bucket/objects/readback/hash verification and local API/detail links passed for agent 2206. **Missing only for the broader submission:** stable deployed origin and deployed-browser walkthrough; no production/mainnet release is claimed. |

CI note: PR #26 (`0353f72`) removed the redundant second web build; the runtime
check reuses the root build. PR #40's merged-main validation is 182/182 web
tests, 5/5 focused result tests, typecheck/lint/build and both CI workflows;
responsive/keyboard checks are UI evidence, not G2 WalletConnect acceptance.

### Immediate critical path

1. **T5/G2 browser:** run the single WalletConnect connector, chain-97 SIWE and sequential ERC-8183 buyer journey with useful 2206 result, exact evidence, explicit terminal decision/review and reload/duplicate/account-change/unknown recovery; never call operator/tunnel evidence browser acceptance.
2. **T3/T9 deployment:** build an immutable accepted SHA, stabilize authorized HTTPS, reinstall/verify cron and smoke browser→API→private PostgreSQL after restart; otherwise retain local/testnet qualifiers.
3. **T9 evidence:** capture current gate results, exact IDs/transactions/digests/URLs, category gaps and three Agent Advantage pairs including trading, stock/equities or security; preserve completed G3/G4 evidence.
4. **T9 submission:** assemble demo/walkthrough, evidence references, honest blockers and rollback/disable commands in the reserved four hours; do not enable release or write mainnet.
...
- [x] **PR #38 account-wide history:** Add authenticated `GET /api/commerce/jobs` with session-derived buyer ownership, bounded newest-first cursor pagination, exact identity/version joins, lifecycle/result/settlement/review summaries, latest public operation and safe next action; reject unauthenticated/address-lookup requests. Cursor precision is stable for equal timestamps.
- [x] **PRs #39–#40 UI:** Ship cohesive marketplace/home/category/detail/compare surfaces, WalletConnect/transaction status and accessible notifications, Creator evidence presentation, `/hired` account-wide filters/pagination/resume UI, stale-account fencing, comparison guidance, exact Creator identity continuity, human-readable result summaries and 320px/200%/keyboard/reduced-motion fixes. No UI change enables gated commerce.
- [x] **Merged-main validation:** Web tests 182/182, focused result tests 5/5, typecheck, lint, production build, responsive/keyboard checks and both CI workflows pass. These checks do not satisfy G2 browser acceptance.
- [x] **Completed account-wide history:** `/hired` reads only the authenticated buyer session, filters returned states, paginates with the opaque cursor, preserves immutable hired identity/version, and resumes a known operation with identity binding. Old/unlisted-agent history remains readable through the persisted job read; no task payloads or secrets are returned.
...
- [ ] **Pending:** Complete the real interactive EOA/SIWE buyer run and recovery evidence above; then capture category gaps and the first Agent Advantage comparison under T9. Keep the quick tunnel canary-only and `releaseEnabled=false`.
...
- [x] **Implemented (PRs #39–#40):** Cohesive shell/navigation, marketplace browse/category/detail/compare, gated hire/status UI, Creator evidence/revoke presentation, `/hired` account-wide history/resume UI, comparison/identity/result refinements and responsive/accessibility checks. This does not pass G2 or public deployment.
- [ ] **Planned:** Run current migrations, build an immutable SHA, establish stable HTTPS, reinstall/verify cron and smoke browser→API→private PostgreSQL after restart.
- [ ] **Planned:** Complete the interactive WalletConnect G2 journey if the browser/RPC prerequisite is available; otherwise preserve the pending gate and `releaseEnabled=false`.
- [ ] **Planned:** Capture current gate evidence, exact IDs/transactions/digests/URLs, three Agent Advantage pairs (including trading, stock/equities or security), demo script, limitations and rollback/disable commands.
...
PRs #39–#40 now carry this read model through a cohesive home, marketplace,
category, detail and compare experience: real cards, URL-backed search/filters,
identity/provenance/six-axis disclosures, comparison scroll guidance and honest
loading/empty/degraded/error states. This is presentation evidence only; it does
not enlarge supply, enable semantic release or prove public deployment.
...
Status: Implemented pending acceptance; T5 WalletConnect-only EOA/SIWE and commerce implementation is merged, the 2206 marketplace/tunnel sub-gate passed, and the operator EOA canary completed a useful paid cycle. Interactive WalletConnect browser acceptance remains. T4 backend/canary is accepted; T5 does not pass the gate until agent 2206 completes the connector/SIWE browser-to-chain useful-result, settlement and review run.
...
Merged-main focused evidence: commerce 54/54, web 182/182, ingestion 115/115, reference-registration/worker scripts 12/12 and focused result summaries 5/5 pass; typecheck, lint, production build, responsive/keyboard checks and both CI workflows pass. PR #38 supplies authenticated buyer-scoped cursor history; PR #39 supplies the cohesive marketplace/Creator/hired UI, stale-account fencing and accessible notifications; PR #40 supplies comparison scroll/identity continuity, readable result summaries and narrow/200%/reduced-motion fixes. The authorized quick-tunnel marketplace evidence also passed: URI transaction `0xe023...c9b` at block `129625704`; finalized sync `129619955–129625757` with 65 observations across 8 identities; 2206 verified/live/published health-factor status with healthy card/invocation; persisted vector/hybrid evidence from the prior run; and the fixed `0.001 U` local ERC-8183 activation offer enabled. These checks plus the operator canary establish implementation and live chain evidence, not the interactive WalletConnect connector/SIWE browser gate; the quick tunnel is canary-only with no uptime guarantee and `releaseEnabled=false` remains.
...
- The `/hired` dashboard and `GET /api/commerce/jobs` are now the account-wide buyer history path: session-derived ownership, newest-first opaque-cursor pagination, filters, safe next actions and resume-by-known-operation. The UI clears stale history on wallet/session changes and never substitutes browser-local history for the account list.
...
Status: Completed for the T8 testnet/local API-detail scope; stable deployed walkthrough and submission remain T9 work.
...
Status: Planned (final acceptance/submission only; P0/P1 UI implementation is complete).

Owner: QA/RELEASE. Prepare in parallel with T8; final verification after all selected gates.

- Run current migrations/build checks, establish stable authorized HTTPS, verify the immutable deployed SHA, reinstall/verify both cron jobs and smoke browser→API→private PostgreSQL after restart.
- Walk through the existing four-category find/compare/understand path, `/hired` account history/resume, one Creator grant/revoke flow and both Greenfield links. Add the G2 paid browser journey only if its WalletConnect evidence passes; do not substitute the operator EOA canary.
- Capture each gate's accepted scope, exact IDs/transactions/digests/URLs, unresolved network/category/access gaps, three Agent Advantage comparisons (one trading, stock/equities or security), rollback/disable commands and submission script.

Exit condition: public/deployed MVP and honest evidence for every claimed feature. Functional gaps block their claim; minor visual polish does not block the demo. No mainnet write or release enablement follows from this plan.
