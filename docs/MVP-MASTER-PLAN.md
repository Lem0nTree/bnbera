...
| G2 | Pending; interactive browser acceptance required | T4–T5 | T4 backend and authorized chain-97 canary are accepted on merged `main`. The T5 2206 tunnel/card/reingestion/publication sub-gate passed; the EOA adapter, browser-to-chain useful-result, settlement and review run remains. |
| G3 | Planned | T6–T7 | Altana passkey/delegated authority is reserved for Creator custody and created-agent execution; no Altana grant/revoke or Agent Studio creation flow is accepted. It is not required to browse or hire an external agent. |
...
| T5 | Pending (EOA adapter/live paid acceptance; 2206 marketplace sub-gate passed) | Browser quote/fund/result/settle/review composition, exact registry fixes, guarded 2206 registration, reference provider and disabled worker are implemented and focused-tested. The branch's passkey bootstrap/recovery remains available as future T6/T7 groundwork, not as the T5 buyer path. Authorized quick-tunnel evidence now verifies 2206's card, invocation, finalized reingestion and publication. | Implement one WalletConnect buyer connector (MetaMask and other compatible wallets are reached through its QR/mobile flow) into one wagmi/viem-style EOA/SIWE boundary, then complete the sequential pinned APEX/ERC-8183 paid journey, provider submit, buyer settle/review and reload/duplicate/account-change/unknown-step recovery evidence. The quick tunnel is canary-only with no uptime guarantee; `releaseEnabled=false` remains. |
...
| --- | --- | --- |
| Frontend | Browse, four category routes, detail, search/filter and compare already exist. The unmerged T5 branch adds authenticated browser commerce, reload/recovery, quote/fund/result/approval/review controls and the passkey bootstrap experiment. The accepted buyer design is one WalletConnect QR/mobile connector (MetaMask and other compatible wallets are selected through WalletConnect) into a wagmi/viem-style EOA boundary and EOA SIWE. | Build and live-test the EOA adapter and sequential paid journey; passkey creation/activation is deferred to T6/T7 and is not a T5 prerequisite |
| Discovery/publication | 8004scan transport and bounded composition runner; T1/T2 are merged. The T5 branch fixes exact ERC-8004 registry-log decoding and identity upsert ordering, and adds a guarded owner-authorized reference-registration harness for the chain-97 reference identity 2206. The authorized quick HTTPS tunnel/card and finalized sync now passed: URI transaction `0xe023...c9b` at block `129625704`, sync `129619955–129625757`, 65 observations across 8 identities, and 2206 verified/live/published as a health-factor service with healthy card/invocation. | Keep the quick tunnel canary-only (no uptime guarantee) and complete the interactive commerce evidence; it is not a final public-preview deployment. |
...
| Hiring | Merged ERC-8183 SDK boundary, PostgreSQL lifecycle/reconciliation, safe APIs and a confirmed chain-97 operator canary. The T5 branch adds quote reservations, browser reload/recovery, persisted result/review projection and a disabled-by-default reference-provider worker. The 2206 marketplace/tunnel sub-gate now passed, including a fixed `0.001 U` local ERC-8183 activation offer. | The EOA adapter must drive each pinned APEX/ERC-8183 transaction sequentially, persist each receipt and reconcile before retry. The interactive run must still show useful callable-agent work, exact result evidence, confirmed settlement and a verified-purchase review. The quick tunnel has no uptime guarantee and `releaseEnabled` remains false. |
| Altana/Creator | The branch's browser passkey bootstrap/reconciliation and server WebAuthn boundary are retained as unaccepted groundwork. Altana is not required for external marketplace discovery, browsing or buyer hiring. | One real bounded Altana grant/action/revoke path in T6, then one Creator template in T7 |
| Greenfield | Publisher abstractions and integrity tests | Pin SDK/provider and publish/read back two real artifacts |

Focused T5 checkout tests passed: `@bnbera/agent-commerce` 54/54, `@bnbera/web` 55/55, `@bnbera/agent-ingestion` 115/115, and the reference-registration/worker scripts 12/12. The authorized quick-tunnel marketplace evidence also passed: URI transaction `0xe023...c9b` at block `129625704`; finalized sync `129619955–129625757` with 65 observations across 8 identities; 2206 verified/live/published health-factor status with healthy card/invocation; and persisted vector/hybrid evidence from the prior run. The fixed `0.001 U` local ERC-8183 activation offer is enabled. These checks do not yet prove the EOA adapter, connector/SIWE account binding, sequential create/register/budget/approve/fund journey, provider submit, 900-second dispute wait, buyer settlement/review or reload/account-change/unknown-step recovery; passkey activation is no longer the T5 acceptance path. The tunnel is canary-only with no uptime guarantee, and `releaseEnabled=false` remains. The old complete-pipeline artifact remains false; new gate evidence must be captured at the accepted implementation SHA.
...
Status: T4 backend/canary complete; the T5 2206 marketplace/tunnel sub-gate passed; G2 remains pending on the EOA adapter and live paid journey. Do not mark G2 complete until an authenticated browser EOA flow drives agent 2206 through escrow, a useful callable result, exact result verification, confirmed settlement and one authenticated verified-purchase review, with reload/duplicate, account/chain-change and unknown-step evidence.
...
The unmerged T5 checkout supplies the browser-owned composition around that boundary: a server-created quote/reservation, explicit buyer funding, reload/recovery, exact result and receipt projection, buyer approval/dispute, settlement and one-per-job verified-purchase review. Its passkey bootstrap and WebAuthn session code is retained as future T6/T7 groundwork, but is not the T5 buyer activation path. T5 must add one WalletConnect QR/mobile buyer connector (MetaMask and other compatible wallets are reached through WalletConnect) into one normalized EOA and SIWE session on chain 97, then sign the pinned APEX/ERC-8183 calls sequentially. Persist the intent and confirmed receipt for every step: `createJob` -> decode the actual `JobCreated` ID -> router `registerJob` -> `setBudget` -> bounded ERC-20 `approve` -> `fund`; after provider work, submit the deliverable and let the buyer EOA explicitly settle, dispute or claim an expiry refund. Reconcile operation-specific events and current job state after every receipt, reload, account/chain change or unknown response before allowing a retry. The provider may retain `@altananetwork/sdk@0.9.0` for its own execution/submission boundary, but buyer signing must not depend on an Altana wallet or passkey. It also supplies a guarded registration harness and reference-provider worker for the health-factor reference identity. The authorized quick HTTPS tunnel/card and finalized reingestion/publication sub-gate now passed for 2206: URI transaction `0xe023...c9b` at block `129625704`, finalized sync `129619955–129625757` with 65 observations across 8 identities, verified/live/published health-factor status with healthy card/invocation, and the prior run's persisted vector/hybrid evidence. The fixed `0.001 U` local ERC-8183 activation offer is enabled while `releaseEnabled=false` remains. The quick tunnel is canary-only with no uptime guarantee and does not constitute final public-preview or G2 acceptance. The remaining live run is the real interactive WalletConnect EOA connection/SIWE authentication and sequential funding journey, provider submit, 900-second dispute wait, buyer settlement/review and reload/duplicate/account-change/unknown-outcome evidence.

The buyer path uses a small WalletConnect-backed wagmi/viem-compatible EOA adapter over the pinned APEX/ERC-8183 ABIs; it is a named, bounded transaction boundary, not arbitrary user calldata or a second escrow. MetaMask and other compatible wallets are selected through WalletConnect QR/mobile, yielding one normalized EOA/SIWE identity. `createJob` must be followed by receipt-derived ID capture, router registration, budget, exact token approval and funding as separate user-controlled sends. The adapter persists a public operation intent before each send and the receipt plus operation-specific event after it, then reconciles by transaction hash/job ID before retrying. `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` is public runtime configuration only and is not a secret or standards-lock pin. The provider may use the pinned Altana SDK for result submission/execution (`submitErc8183Deliverable`, `getErc8183Job` and supported provider-side calls); the Altana MCP remains an AI-host wrapper, not a BNBEra runtime dependency. Do not route buyer hire through `hireErc8183Agent`'s atomic relay or require an Altana smart wallet/passkey.
...
User flow: select agent -> connect WalletConnect with MetaMask or another compatible wallet -> establish EOA/SIWE on BSC testnet -> enter task -> review quote/budget -> create the job and follow the sequential buyer sends (`createJob`, actual ID capture, router registration, budget, bounded approval, fund) -> agent performs work -> provider submits the result -> buyer verifies exact canonical bytes and explicitly approves or disputes -> settlement confirmed, or the buyer claims an expiry refund. Persist job ID, buyer/provider, full agent identity/version, quote, status, each step's intent/receipt, transaction hashes, local evidence digest, on-chain deliverable Keccak and result URL. Use IPFS only if the selected Studio/ERC-8183 deliverable path needs it; Greenfield comes later.
...
**Gate G2:** one real authorized paid cycle passes from the WalletConnect browser connector (with MetaMask or another compatible wallet) through sequential EOA escrow calls to a useful result and confirmed settlement, including SIWE, page reload, duplicate-submit protection and account/chain-change handling; one negative/unknown-step recovery is verified. Completed-job count and buyer review appear in the profile. Then prove the same usable activation/result flow for each category selected for the main-track demo; record category gaps explicitly.

## G3 — No-code creation with Altana

Status: planned (T6–T7). The T5 browser passkey bootstrap is not required for browsing or hiring and is not a G2 prerequisite; it is retained only as possible groundwork for this Creator milestone. No Altana grant, revocation/deny path or Agent Studio deployment is accepted.
...
Before demo: public production build, persistent DB and cron, all four categories with useful real supply and activation evidence, one paid cycle, one Creator flow and the required three-task Agent Advantage Report. Add the two Greenfield links only after that core path works; x402/B402 remains disabled and out of scope. Run relevant build/tests plus one real browser walkthrough. Do not wholesale-revert the latest T5 browser changes or reset migrations/retained data: preserve the 2206 tunnel/pipeline evidence, canonical commerce history and passkey code as future T6/T7 groundwork, and roll back only an application artifact if a safe disable is needed. Back up the retained DB before migrations, verify any changed migration on disposable data, and preserve the existing forward-repair history. Keep all six state axes and secret-reference boundaries. Remaining cosmetic polish is deferred.
