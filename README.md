...
The [Smart Money Era main track](https://www.bnbchain.org/en/hackathons/smart-money-era?tab=tracks) asks for a BNB Agent Studio marketplace built around discovery, useful data and activation. BNBEra connects those steps through category discovery, enriched profiles, signed quotes and escrow-backed hiring.
...
See the [Creator and storage test report](docs/release-evidence/mainnet-e2e-2026-09-09/README.md) for validation details.
...
| --- | --- |
| ✅ **Main Track — BNB Agent Studio Marketplace** | Discover, compare and hire BSC agents across all four categories, using enriched profiles and verifiable work evidence. |
| ✅ **Altana** | Agent-owned wallets, scoped sessions with spend limits and expiry, owner-controlled revocation, and ERC-8183 provider integration. [Testnet evidence](docs/MVP-TASKS.md#g3--no-code-creator-with-altana-t6t7). |
| ✅ **PancakeSwap** | A no-code agent template for bounded swaps with curated pairs, exact input, slippage limits and deadlines. [Template](templates/pancakeswap-one-shot). |
